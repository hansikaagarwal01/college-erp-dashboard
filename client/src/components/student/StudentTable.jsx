import { MdVisibility, MdEdit, MdDelete, MdChevronLeft, MdChevronRight } from "react-icons/md";
...
const avatarVariants = [
  "bg-primary-fixed text-on-primary-fixed dark:bg-primary-500/30 dark:text-primary-100",
  ...
  "bg-primary-fixed-dim text-on-primary-fixed-variant dark:bg-primary-500/30 dark:text-primary-100",
];
...
function yearLabel(student) {
  const year = student.semester ? Math.ceil(Number(student.semester) / 2) : 0;
  if (year >= 1 && year <= 4) {
    const ordinal = ["1st", "2nd", "3rd", "4th"][year - 1];
    return `${ordinal} Year`;
  }
  if (student.batch) return `${student.batch} Batch`;
  return "—";
}
  ...
  if (students.length === 0) {
    return (
      <div className="rounded-xl border border-outline-variant bg-surface-container-lowest shadow-[0px_1px_3px_rgba(0,0,0,0.05),0px_10px_15px_-3px_rgba(0,0,0,0.02)] dark:border-gray-700/60 dark:bg-gray-900">
        <EmptyState
          message={hasFilters ? "No students match your search" : "No students found"}
          hint={
            hasFilters
        ...
        <table className="w-full border-collapse text-left">
          <thead>
            <tr className="border-b border-outline-variant bg-[#f1f5f9] dark:border-gray-700/60 dark:bg-gray-800/70">
              <th className="sticky left-0 bg-[#f1f5f9] px-4 py-3 text-xs font-semibold uppercase tracking-wider text-on-surface-variant dark:bg-gray-800/70 dark:text-gray-400">
                Student Name
              </th>
          ...
          <tbody>
            {students.map((student, index) => (
              ...
              >
                <td className="sticky left-0 bg-surface-container-lowest px-4 py-4 transition-colors group-hover:bg-surface-container-low dark:bg-gray-900 dark:group-hover:bg-gray-800/60">
